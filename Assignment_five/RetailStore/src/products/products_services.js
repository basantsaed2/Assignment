...
const createProduct = (req, res) => {
    ...
    db.query(query, (err, results) => {
        if (err) {
            ...
            res.status(201).json({ message: "Product created successfully", product: results[0] });
        }
    });
};
...
const updateProduct = (req, res) => {
    ...
    const query = `UPDATE ${ProductSchema.tableName} SET ${ProductSchema.attributes.name} = '${name}', ${ProductSchema.attributes.price} = '${price}', ${ProductSchema.attributes.stock} = '${stock}', ${ProductSchema.attributes.supplier} = '${supplier}' WHERE ${ProductSchema.attributes.id} = ${id}`;
    db.query(query, (err, results) => {
        if (err) {
